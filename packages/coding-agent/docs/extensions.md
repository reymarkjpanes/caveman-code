...
| Package | Purpose |
|---------|---------|
| `cave` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `@sinclair/typebox` | Schema definitions for tool parameters |
| `@cave/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@cave/tui` | TUI components for custom rendering |
...
```typescript
import { isBashToolResult } from "cave";
...
```typescript
import { createLocalBashOperations } from "cave";
...
```typescript
import { SessionManager } from "cave";
...
```typescript
import type { AutocompleteItem } from "@cave/tui";
...
```typescript
import { withFileMutationQueue } from "cave";
...
**Important:** Use `StringEnum` from `@cave/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "cave";
...
```typescript
import { Text, Component } from "@cave/tui";
...
import { matchesKey } from "@cave/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "cave";
