...
The `Agent` class (from `@cave/agent`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "cave";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "cave";
...
import { createAgentSession, defineTool } from "cave";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "cave";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "cave";
