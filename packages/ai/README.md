...
TypeBox exports are re-exported from `@cave/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@cave/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@cave/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@cave/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@cave/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@cave/ai';
...
```typescript
import { getModel, stream } from '@cave/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@cave/ai';
...
```typescript
import { Model, stream } from '@cave/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@cave/ai';
...
```typescript
import { getModel, complete, Context } from '@cave/ai';
...
```typescript
import { Context, getModel, complete } from '@cave/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@cave/ai/oauth` entry point in Node.js.
...
npx @cave/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@cave/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@cave/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@cave/ai`
