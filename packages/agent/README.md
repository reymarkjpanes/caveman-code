...
Stateful agent with tool execution and event streaming. Built on `@cave/ai`.

## Installation

```bash
npm install @cave/agent
```
...
```typescript
import { Agent, streamProxy } from "@cave/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@cave/agent";
