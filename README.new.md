...
Four compression layers. Always on. Pays for itself after one tool call.

| Layer | What happens | Cut |
|---|---|---|
| **Caveman Mode** | Model answers in terse technical fragments — no filler, no hedging. Levels: `lite` · `full` · `ultra`. | prompt + reply |
| **Tool Budgets** | Per-tool line caps (bash 80 · read 300 · grep 120), ANSI strip, blank-line collapse, semantic JSON/XML extraction. | **−67% to −94%** on tool output |
| **Read Dedup** | Files are fingerprinted per session — re-reads return a stub, not the bytes. | **−99%** on repeat reads |
| **RTK** | Optional Rust binary rewrites bash output before it ever enters context. | **~60%** extra |
...
| Plan mode (read-only chat) | ✅ | partial | ❌ | ❌ | ❌ |
...
| `~/.claude/skills/<n>/SKILL.md` | `~/.cave/skills/<n>/SKILL.md` | Identical |
| `~/.claude/agents/<n>.md` | `~/.cave/agents/<n>.md` | Frontmatter superset |
...
**Caveman Code is a heavy fork of [pi-code](https://github.com/badlogic/pi-code) by [Mario Zechner](https://github.com/badlogic).** The agent runtime, MCP scaffolding, provider OAuth flows, repo map, slash-command parser, settings manager, skills loader, edit-format renderers, and TUI components all originate upstream. We track `pi-code` and contribute fixes back where generally useful.

Caveman Code's own work, layered on top: **Caveman Mode** (4-layer compression) · **goal loop** (autonomous Ralph runner) · **plan mode** · **cavemem** memory integration · **session branching `/tree`** · **architect/editor split** · **honest-bench** eval harness.
